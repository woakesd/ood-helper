...
{
    /// <summary>
    /// The race (calendar) list: find-by-event filter + Add/Edit/Delete. Replaces the old
    /// Races code-behind. Mirrors <see cref="SeriesViewModel"/>.
    /// </summary>
    public partial class RacesViewModel : ObservableObject
    {
        ...
        public void Load()
        {
            ...
            Rows = items == null ? null : new ObservableCollection<Calendar>(items);
        }

        [RelayCommand]
        private void Add()
        {
            if (_dialogs.ShowRaceEditor(0))
                Load();
        }
        ...
        private void Edit()
        {
            ...
            if (_dialogs.ShowRaceEditor(SelectedRow.Rid))
                Load();
        }
        ...
        private void Delete(IList selectedItems)
        {
            ...
            var change = false;
            foreach (var item in selectedItems.Cast<Calendar>().ToArray())
            {
                bool? result = _dialogs.ConfirmYesNoCancel(
                    "Are you sure you want to delete " + item.Event + "?", "Confirm Delete");
                if (result == null) break;
                if (result == true)
                {
                    _repository.Delete(item.Rid);
                    change = true;
                }
            }
