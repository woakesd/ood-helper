...
namespace OodHelper.net
{
    ...
    class RollingHandicap : RaceScore
    {
        ...
        public void Calculate(int r)
        {
            ...
            p["rid"] = rid;
            Db c = new Db(@"SELECT spec, date, hc
                        FROM calendar
                        WHERE rid = @rid");
        ...
        private void CorrectedTime()
        {
            ...
            //
            sql = @"SELECT bid, rid, bstart, fintime, hcap, ohp, laps, elapsed, corrected, stdcorr, place
                FROM races
                WHERE rid = @rid";
            c = new Db(sql);
            ...
            {
                if (dr["bstart"] != DBNull.Value && dr["fintime"] != DBNull.Value && dr["laps"] != DBNull.Value)
                {
                    string start = dr["bstart"].ToString();
                    ...
                    if (r1.Match(start).Success && r1.Match(ftime).Success)
                    {
                        ...
                        if (racetype == "a")
                        {
                            ...
                            dr["stdcorr"] = Math.Round(e.TotalSeconds * 1000 / ohp / l);
                        }
                        else
                        {
                            dr["corrected"] = Math.Round(e.TotalSeconds * 1000 / hcap);
                            dr["stdcorr"] = Math.Round(e.TotalSeconds * 1000 / ohp);
                        }
                        dr["place"] = 0;
                    }
                }
        ...
        private void CalculateSct()
        {
            ...
            //
            Db c = new Db(@"SELECT rid, bid, corrected, stdcorr, a
                    FROM races
                    WHERE rid = @rid
                    AND ohstat IN ('PY', 'SY', 'RN', 'CN')
                    AND place = 0
                    ORDER BY stdcorr");
            Hashtable p = new Hashtable();
            p["rid"] = rid;
            ...
            {
                if (useStd)
                    total = total + (double) d.Rows[i]["stdcorr"];
                else
            ...
            for (int i = 0; i < d.Rows.Count; i++)
            {
                ...
                row["a"] = "N";
                if (((double) row["stdcorr"]) < AvgSlowLimit)
                {
                    row["a"] = DBNull.Value;
                    goodBoats++;
                    if (useStd)
                        standardCorrectedTime = standardCorrectedTime + (double)row["stdcorr"];
                    else
            ...
            Db up = new Db(@"UPDATE calendar SET sct = @sct, r = 1 WHERE rid = @rid");
        ...
        private void Score()
        {
            ...
            Db c = new Db(@"SELECT bid, rid, corrected, place, pts
                    FROM races
                    WHERE rid = @rid
                    AND place = 0
                    ORDER BY corrected");
        ...
        private void UpdateHandicaps()
        {
            ...
            Db c = new Db(@"SELECT bid, rid, bstart, stdcorr, hcap, ohp, achhc, newhc, c
                    FROM races
                    WHERE rid = @rid
                    AND place <> 999
                    ORDER BY corrected");
            ...
            foreach (DataRow dr in d.Rows)
            {
                ...
                if (standardCorrectedTime > 0)
                {
                    ...
                    // the open handicap.
                    //
                    int achhc = (int) Math.Round((double)dr["stdcorr"] / standardCorrectedTime * (int) dr["ohp"]);
                    dr["achhc"] = achhc;
                    ...
                    if ((double) dr["stdcorr"] > SlowLimit || (double) dr["stdcorr"] < FastLimit)
                    {
                        if ((double) dr["stdcorr"] > SlowLimit)
                        {
                            dr["c"] = "s";
                            sperf = true;
                        }
                        ...
                        param["bid"] = dr["bid"];
                        param["rid"] = rid;
                        TimeSpan bstart = Common.tspan(dr["bstart"].ToString()).Value;
                        DateTime bdate = rdate + bstart;
                        param["bstart"] = bdate;
                        Db sl = new Db(@"SELECT TOP(1) CONVERT(FLOAT,(achhc - ohp))/ohp * 100
                            FROM races
                            WHERE bid = @bid
                            AND rid != @rid
                            AND place != 999
                            AND CONVERT(DATETIME,SUBSTRING(CONVERT(NCHAR(19),date),1,11) + ' ' + replace(bstart,' ',':'))
                                <= @bstart
                    ...
                    if (!sperf || sperfover)
                    {
                        ...
                        //
                        int working = achhc;
                        if (achhc > (int) dr["ohp"] * 1.05)
                            working = (int) Math.Round(1.05 * (int) dr["ohp"], 0);
                        if (achhc < (int) dr["ohp"] * 0.95)
                            working = (int) Math.Round(0.95 * (int)dr["ohp"], 0);

                        //
                        // Move the rolling handicap 15% towards the achieved handicap.
                        //
                        int newhc = (int) Math.Round((int) dr["hcap"] + (working - (int) dr["hcap"]) * 0.15);

                        //
                        // And keep it if it's inside the band.
                        //
                        if (newhc >= (int) dr["hcap"] * 0.95 && newhc <= (int) dr["hcap"] * 1.05)
                            dr["newhc"] = newhc;
                    }
                }
            }
            ...
            c = new Db(@"SELECT bid, newhc
                    FROM races
                    WHERE rid = @rid");
            d = c.GetData(p);

            Db u = new Db(@"SELECT bid, rhp
                    FROM boats
